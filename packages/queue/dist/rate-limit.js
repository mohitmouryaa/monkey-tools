 ...
 */
export async function checkRateLimit(redis, key, config) {
 ...
 */
export async function checkMultiTierRateLimit(redis, key, tiers) {
 ...
 */
export async function getRateLimitStatus(redis, key, config) {
    const { limit, window, keyPrefix = "ratelimit" } = config;
    try {
        const now = Math.floor(Date.now() / 1000);
        const windowKey = `${keyPrefix}:${key}:${Math.floor(now / window)}`;
 ...
 * @param config - Rate limit configuration
 */
export async function resetRateLimit(redis, key, config) {
