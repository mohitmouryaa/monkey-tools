...
export default function RotatePDF() {
  ...
  const handleRotateSelected = (direction: "cw" | "ccw") => {
    if (selectedPages.size === 0) {
      toast.error("Selecione páginas para rotacionar.");
      return;
    }
  ...
  const savePDF = useCallback(async () => {
    ...
    try {
      ...
      console.error(error);
      toast.error("Ocorreu um erro ao salvar o PDF.");
    } finally {
      setIsSaving(false);
    }
  }, [files, pages]);
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 p-4 border rounded-xl bg-card">
              ...
              <div className="flex items-center gap-2">
                {selectedPages.size > 0 && (
                  <>
                    <Button variant="outline" size="sm" onClick={() => handleRotateSelected("ccw")} title="Girar à esquerda">
                      <RotateCcw className="w-4 h-4" />
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => handleRotateSelected("cw")} title="Girar à direita">
      ...
      {isSaving && (
        <div className="max-w-3xl mx-auto mt-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium">Salvando PDF...</span>
