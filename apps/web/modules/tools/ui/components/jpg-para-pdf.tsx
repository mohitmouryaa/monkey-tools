...
export default function JPGToPDF() {
  ...
  const handleAdditionalFiles = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = newFiles.filter((file) => {
      if (!file.type.startsWith("image/jpeg") && !file.type.startsWith("image/jpg")) {
        toast.error(`${file.name} não é uma imagem JPG`);
        return false;
      }
  ...
  const convertToPdf = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Falha na conversão");
    } finally {
    ...
    <div className="w-full">
      ...
      <section aria-labelledby="upload-section" className="max-w-3xl mx-auto">
        {files.length === 0 ? (
          <FileUpload
            ...
            label="Enviar Imagens JPG"
            description="Selecione múltiplas imagens JPG para converter em PDF"
            disclaimer="Imagens são processadas com segurança e não armazenadas em nossos servidores"
          />
        ) : (
          <div className="space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Tamanho total: {formatFileSize(files.reduce((sum, f) => sum + f.file.size, 0))}
                  </p>
                </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">As imagens serão convertidas em páginas PDF na ordem mostrada acima</p>
            </div>
              ...
              <div className="space-y-4">
                <div className="p-6 border-2 border-green-300 shadow-lg rounded-xl bg-linear-to-br from-green-50 to-green-100 dark:from-green-950/30 dark:to-green-900/20 dark:border-green-700">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-green-800 dark:text-green-200">Conversão Concluída!</h4>
                      <p className="text-sm text-green-700 dark:text-green-300">Seu PDF está pronto para download</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-4 mb-6 md:grid-cols-3">
                    <div className="p-3 text-center rounded-lg bg-white/50 dark:bg-black/20">
                      ...
                      <p className="text-sm text-muted-foreground">Imagens Convertidas</p>
                    </div>
                    <div className="p-3 text-center rounded-lg bg-white/50 dark:bg-black/20">
                      <p className="text-2xl font-bold text-green-600">{conversionResult.imageCount}</p>
                      <p className="text-sm text-muted-foreground">Páginas PDF</p>
                    </div>
                    <div className="p-3 text-center rounded-lg bg-white/50 dark:bg-black/20">
                      <p className="text-2xl font-bold text-green-600">{formatFileSize(conversionResult.blob.size)}</p>
                      <p className="text-sm text-muted-foreground">Tamanho do Arquivo</p>
                    </div>
                  </div>
      ...
      {isConverting && (
        <div className="max-w-3xl mx-auto mt-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium">Convertendo imagens para PDF...</span>
      ...
      <Alert className="max-w-3xl mx-auto mt-6 text-yellow-800 border-yellow-200 bg-yellow-50 dark:bg-yellow-950/20 dark:border-yellow-800 dark:text-yellow-200">
        ...
        <AlertDescription className="text-yellow-700 dark:text-yellow-300">
          As imagens são convertidas na ordem em que foram enviadas. Certifique-se de enviá-las na sequência desejada.
        </AlertDescription>
      </Alert>
    </div>
  );
