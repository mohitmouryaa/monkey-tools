...
export default function AddPageNumberPDF() {
  ...
  const handleFileSelect = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      toast.error("Falha ao carregar PDF. Tente novamente.");
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      toast.success("PDF baixado com números de página!");
    } catch (error) {
      console.error("Erro ao processar o PDF:", error);
      toast.error("Falha ao processar PDF. Tente novamente.");
    } finally {
      setIsProcessing(false);
    }
  };
    ...
    return (
      <div className="w-full max-w-4xl mx-auto">
        <FileUpload
          ...
          maxFiles={1}
          maxFileSize={150}
          label="Adicionar Números de Página ao PDF"
          description="Envie um arquivo PDF para adicionar números de página"
        />
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              {isUpdatingPreview && (
                <div className="absolute inset-0 z-20 flex items-center justify-center backdrop-blur-sm">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-muted-foreground">Renderizando prévia...</span>
                  </div>
                </div>
              )}
              {previewUrl ? (
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="flex flex-col h-full p-6 space-y-6 border rounded-lg shadow-sm">
            <h3 className="flex items-center gap-2 text-lg font-semibold">Configurações</h3>

            <div className="flex-1 space-y-6">
              {/* Position */}
              <div className="space-y-3">
                <Label className="text-xs tracking-wider uppercase text-muted-foreground">Posição</Label>
              ...
              <div className="space-y-4">
                <Label className="text-xs tracking-wider uppercase text-muted-foreground">Tipografia</Label>
              ...
              <div className="pt-2 space-y-4 border-t">
                <Label className="text-xs tracking-wider uppercase text-muted-foreground">Numeração</Label>

                <div className="space-y-2">
                  <Label className="text-xs">Iniciar sequência a partir de</Label>
                  <Input
                    type="text"
                    inputMode="numeric"
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Se definido como 5, a primeira página será numerada "5", a próxima "6", etc.
                  </p>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label className="text-xs">Aplicar a Partir da Página</Label>
                    <Input
                      value={fromInput}
                      onChange={(e) => {
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs">Até a Página</Label>
                    <Input
                      value={toInput}
                      onChange={(e) => {
                        const val = e.target.value;
            ...
            <div className="pt-4 space-y-3 border-t">
              ...
              <Button onClick={resetAll} variant="ghost" className="w-full text-muted-foreground">
                ...
                Reiniciar e Enviar Novo Arquivo
              </Button>
            </div>
          </div>
        </div>
