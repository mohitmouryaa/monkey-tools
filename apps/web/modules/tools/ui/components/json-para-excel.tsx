...
export default function JsonToExcelConverter() {
  ...
  const handleJsonParse = useCallback((input: string, sourceName = "data") => {
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        throw new Error("O JSON precisa ser um array ou um objeto.");
      }

      if (generatedSheets.length === 0) {
        throw new Error("Nenhum dado válido encontrado para converter.");
      }
      ...
      setError(null);
      toast.success(`${generatedSheets.length} planilha(s) processada(s) com sucesso!`);
    } catch (err) {
  ...
  const processInput = () => {
    if (!jsonInput.trim()) {
      setError("Insira algum dado JSON.");
      return;
    }
      ...
      <div className="max-w-4xl mx-auto space-y-12">
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <h2 className="text-2xl font-bold tracking-tight">1. Inserir Dados JSON</h2>
            <p className="text-muted-foreground">
              Cole seus dados JSON abaixo ou envie um arquivo .json para converter em uma planilha Excel.
            </p>
            ...
            {error && (
              <Alert variant="destructive">
                <AlertTitle>Erro ao Processar JSON</AlertTitle>
          ...
          <div className="space-y-4">
            <h2 className="text-2xl font-bold tracking-tight">2. Prévia e Exportação</h2>
            <p className="text-muted-foreground">Verifique a estrutura dos dados antes de baixar.</p>
              ...
              <div className="flex flex-col p-0">
                <div className="flex items-center justify-between p-4 border-b bg-muted/30">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-semibold">Prévia dos Dados</h3>
                  </div>
                  {sheets && activeSheet && (
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 font-mono text-xs font-medium rounded-full bg-primary/10 text-primary border border-primary/20">
                        {activeSheet.data.length} Linhas
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="relative overflow-auto min-h-75 max-h-125 bg-background">
                  {sheets && activeSheet ? (
                    <div className="min-w-max">
                      ...
                      {activeSheet.data.length > 5 && (
                        <div className="sticky bottom-0 left-0 right-0 p-3 text-xs font-medium text-center border-t bg-background/95 backdrop-blur-sm text-muted-foreground">
                          E mais {activeSheet.data.length - 5} linhas...
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="flex flex-col items-center justify-center w-full h-75 text-muted-foreground">
                      ...
                      <p className="text-sm opacity-70">Importe JSON para ver a prévia</p>
                    </div>
                  )}
                </div>
