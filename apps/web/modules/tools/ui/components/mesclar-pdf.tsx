...
export default function MergePDF() {
  ...
  const mergePDFs = useCallback(async () => {
    if (pages.length === 0) {
      toast.error("Nenhuma página para mesclar.");
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        if (!fileItem.file.size || fileItem.file.size > MAX_FILE_SIZE) {
          toast.error(`Arquivo "${fileItem.file.name}" é muito grande para mesclar. Tamanho máximo: 50MB.`);
          setIsMerging(false);
          return;
        }
      ...
      toast.error("Ocorreu um erro ao mesclar os PDFs.");
    } finally {
      setIsMerging(false);
    }
  }, [files, pages]);
  ...
  return (
    <div className="w-full">
      ...
      {isMerging && (
        <div className="max-w-3xl mx-auto mt-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium">Mesclando Páginas...</span>
      ...
      <div className="flex flex-col justify-center gap-4 mt-8 sm:flex-row">
        ...
        >
          {isMerging ? "Mesclando..." : "Mesclar PDF"}
        </Button>
