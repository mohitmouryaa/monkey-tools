...
export const PagesPagination = () => {
  ...
  const page = pagesQuery.data?.page ?? params.page;
  const totalPages = pagesQuery.data?.totalPages ?? 0;
  const totalCount = pagesQuery.data?.totalCount ?? 0;
  const pageSize = pagesQuery.data?.pageSize ?? params.pageSize;

  const start = totalCount === 0 ? 0 : (page - 1) * pageSize + 1;
  const end = Math.min(page * pageSize, totalCount);
  const isBusy = pagesQuery.isFetching;
    ...
    <div className="flex flex-col items-start justify-between gap-3 border-t border-border pt-4 sm:flex-row sm:items-center">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <Select value={String(pageSize)} onValueChange={(value) => setParams({ ...params, pageSize: Number(value), page: 1 })}>
