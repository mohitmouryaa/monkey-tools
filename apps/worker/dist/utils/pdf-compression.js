...
export class AdvancedPdfCompressor {
        ...
        return new Promise((resolve, reject) => {
            ...
            proc.on("close", (code) => {
                ...
                if (code === 0)
                    ...
                    reject(new Error(`${command} failed with code ${code}: ${Buffer.concat(stderr).toString()}`));
            });
        });
    }
    ...
    async step1Reconstruct(inputPath, outputPath) {
        const args = [
            "-sDEVICE=pdfwrite",
    ...
    async step2Optimize(inputPath, outputPath) {
        const args = [
            inputPath,
            outputPath,
    ...
    async step3Extreme(inputPath, outputPath) {
        const args = [
            "-sDEVICE=pdfwrite",
            "-dPDFSETTINGS=/screen",
    ...
    async removeMetadata(inputPath) {
    ...
    async compress(inputPath, outputPath, preset = "medium") {
