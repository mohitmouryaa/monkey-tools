...
> Versão: v2 | Criado em: 2026-04-30 | Status: draft
> Revisado por subagente | 3 sugestões incorporadas, 0 ignoradas
...
- **RSC vs Client**: o dashboard usa Server Components extensivamente após
  o refactor recente (commit `d9b9e40`). `dashboard-overview-view.tsx` é
  **RSC** (sem `"use client"`). Portanto, qualquer `next/dynamic({ ssr: false })`
  precisa viver dentro de um Client Component intermediário.
...
3. `optimizePackageImports` está estável no Next 16 (saiu de `experimental`).
   Posicionar no nível raiz do config; se Next reclamar, mover para
   `experimental.optimizePackageImports`. Funciona via SWC para
   `lucide-react` e tem suporte a `zod` (testado no Next 15+).
4. Os schemas Zod usados em `tool-form.tsx`, `account-tabs.tsx`,
   ...
   por inspeção. Não há refatoração de mover pra server: schemas Zod precisam
   estar no client para `zodResolver` funcionar.
...
7. `PostModel` em `packages/database/src/models/Post.ts` declara
   ...
   declarado dentro de `getPostModel()` que executa toda vez que o módulo
   é (re)avaliado.
8. **Verificado por inspeção**: `pdf-para-excel.tsx` NÃO importa xlsx, jspdf
   ou pdf-lib eager — só faz upload via tRPC. Foi removido do escopo da
   Fase 3 (era falso-positivo do grep inicial).
...
#### 1.1 — Adicionar `optimizePackageImports` em next.config.mjs
**Arquivo:** `apps/web/next.config.mjs`
**Depende de:** —
Adicionar `optimizePackageImports: ["lucide-react", "zod"]` no objeto `nextConfig`. Em Next 16 a opção é estável; tentar primeiro no nível raiz. Se Next reclamar com warning de "unknown option", mover para `experimental: { optimizePackageImports: [...] }`. Não tocar em `transpilePackages`.
...
**Depende de:** 1.1
Rodar `ANALYZE=true pnpm --filter web build`. Se o `next.config.mjs` ainda não tem o wrapper de bundle-analyzer, adicionar o wrapper condicional (`withBundleAnalyzer({ enabled: process.env.ANALYZE === "true" })`) ao mesmo arquivo. Comparar tamanho de chunk lucide-react antes/depois e anotar no PR.
...
um mapa estático com chunks por slug.
...
> ATENÇÃO: esta fase tem decisão de implementação marcada como [DECISÃO PENDENTE]
> mas é executável com defaults. A decisão "incluir todas as tools no registry"
> é arquitetural por afetar manutenção a longo prazo (registry crescer junto
> com a pasta de tools).
...
#### 2.2 — Reescrever tool-embed-block-renderer para usar o registry ✅
**Arquivo:** `apps/web/modules/blog/ui/components/blocks/tool-embed-block-renderer.tsx`
**Depende de:** 2.1
Trocar o `await import(\`@/modules/tools/ui/components/${componentSlug}\`)` por lookup no `TOOL_EMBED_REGISTRY[componentSlug]`. Manter como async Server Component e chamar `const mod = await registry[slug]?.()`. Se o slug não estiver no registry, retorna null. Mantém o `Suspense` + `PDFLibProvider` atuais.
**Feito quando:** post com tool embed renderiza no dev e em prod build, e analyzer mostra que o chunk inicial de `/blog/[slug]` NÃO contém código de tool (verificável buscando por nome de função distinta de uma tool no chunk).

#### 2.3 — Documentar tamanho antes/depois no PR ✅
**Arquivo:** N/A
**Depende de:** 2.2
...
Tools confirmadas com import eager (verificadas por inspeção):

- [x] `excel-para-pdf.tsx` — `import * as XLSX` + `import { jsPDF }` → mover ambos para dentro do handler de conversão
- [x] `marca-dagua-pdf.tsx` — dynamic de `pdf-lib`
- [x] `pdf-para-jpg.tsx` — dynamic de `pdf-lib` (`pdfjs-dist` já é dynamic)
- [x] `numerar-pdf.tsx` — dynamic de `pdf-lib`
- [x] `rotacionar-pdf.tsx` — dynamic de `pdf-lib`
- [x] `mesclar-pdf.tsx` — dynamic de `pdf-lib`
- [x] `json-para-excel.tsx` — dynamic de `xlsx`
- [x] `dividir-pdf.tsx` — dynamic de `pdf-lib` (jszip não usado neste arquivo — confirmado por grep)
- [x] `jpg-para-pdf.tsx` — dynamic de `jspdf`
- [ ] Validar manualmente cada tool após mudança (uma rodada de smoke test) — **handoff ao usuário** (não executável via terminal)

> Nota: `pdf-para-excel.tsx` foi removido do escopo. Verificado por inspeção
> que ele só faz upload via tRPC e não importa xlsx/jspdf/pdf-lib eager.
...
> ATENÇÃO ARQUITETURAL: `dashboard-overview-view.tsx` é **Server Component**.
...
**Arquivo:** `apps/web/next.config.mjs`
**Depende de:** —
path-to-regexp v8 não suporta `:param.(group)` na mesma sintaxe antiga. [DECISÃO PENDENTE 6.2]: qual sintaxe usar? **Default razoável**: usar `source: "/:path(.*\\.(svg|png|jpg|jpeg|gif|webp|avif|ico|woff|woff2|ttf|otf))"` (regex inline em named param), que é o padrão sugerido pela doc Next 16. Se falhar, partir para um entry por extensão (`/:path*.svg`, `/:path*.png`, etc.).
...
| `next/dynamic({ ssr: false })` em RSC quebra build | Alta se ignorada | **Mitigado por Fase 4.1**: criar wrapper Client Component intermediário antes de aplicar dynamic |
...
**Fase 4**
- `apps/web/modules/dashboard/ui/components/dashboard-overview-charts.tsx` (criar — wrapper client)
- `apps/web/modules/dashboard/ui/views/dashboard-overview-view.tsx` (modificar — RSC consome wrapper)
