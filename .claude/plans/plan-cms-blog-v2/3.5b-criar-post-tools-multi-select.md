...
# ✅ 3.5b — Criar `post-tools-multi-select.tsx`

## Contexto
Decisão P5: Popover + Command (cmdk) com checkboxes e busca para selecionar múltiplas tools (`toolIds: string[]`).

## Detalhe técnico
Criar `apps/web/modules/dashboard/ui/components/post-tools-multi-select.tsx`:
...
Notas:
- `pageSize: 100` — assume que o universo de tools é gerenciável; se ultrapassar, evoluir para servidor com search (fora de escopo).
- Largura do Popover acompanha o trigger via `w-[--radix-popover-trigger-width]`.

## Arquivos tocados
- `apps/web/modules/dashboard/ui/components/post-tools-multi-select.tsx` — criar.

## Verificação
- [ ] `pnpm --filter web typecheck` passa.
- [ ] Componente busca tools, exibe checkmark, e propaga `onChange` corretamente.

## Decisões aplicadas
- **P5 (A)**: Popover + Command com checkboxes/busca.
