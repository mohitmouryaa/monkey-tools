---
id: 3.1d
phase: 3
type: criar
depends_on: [3.1b]
plan_file: plan-cms-blog-v2.md
---

# ✅ 3.1d — Criar hook `use-posts-search` (debounce dedicado)

## Contexto
Análogo dedicado de `useEntitySearch`. Debounce de 500ms entre digitar no input e atualizar params. Reseta `page` para 1 ao buscar. Código próprio (regra de não-reuso).

## Detalhe técnico
Criar `apps/web/modules/dashboard/hooks/use-posts-search.tsx`:

```tsx
"use client";

import { useEffect, useState } from "react";
import { PAGINATION } from "@/modules/common/constants";
import { usePostsParams } from "@/modules/dashboard/hooks/use-posts-params";

const DEBOUNCE_MS = 500;

export const usePostsSearch = () => {
  const [params, setParams] = usePostsParams();
  const [localSearch, setLocalSearch] = useState(params.search ?? "");

  useEffect(() => {
    if (localSearch === "" && params.search !== "") {
      setParams({ ...params, search: "", page: PAGINATION.DEFAULT_PAGE });
      return;
    }
    const timer = setTimeout(() => {
      if (localSearch !== params.search) {
        setParams({ ...params, search: localSearch, page: PAGINATION.DEFAULT_PAGE });
      }
    }, DEBOUNCE_MS);
    return () => clearTimeout(timer);
  }, [localSearch, params, setParams]);

  useEffect(() => {
    setLocalSearch(params.search ?? "");
  }, [params.search]);

  return { searchValue: localSearch, onSearchChange: setLocalSearch };
};
```

## Arquivos tocados
- `apps/web/modules/dashboard/hooks/use-posts-search.tsx` — criar.

## Verificação
- [ ] `pnpm --filter web typecheck` passa.
- [ ] Hook não importa `useEntitySearch` de `modules/common`.

## Decisões aplicadas
- Regra de não-reuso: análogo dedicado de `useEntitySearch`.
