...
const STOPWORDS_EN = new Set([
  "the","is","are","was","were","be","been","being","am","has","have","had",
  "do","does","did","doing","will","would","can","could","should","shall","may","might","must",
  "of","to","in","on","at","by","with","from","for","about","into","onto","over","under","between","through",
  "and","or","but","if","then","else","when","while","because","so","than","as","not","no","yes",
  "this","that","these","those","there","here","where","what","which","who","whom","whose","why","how",
  "your","our","their","my","his","her","its","you","we","they","i","me","us","them","him","he","she","it",
  "click","upload","download","drag","drop","choose","select","convert","compress","merge","split","rotate",
  "page","pages","file","files","image","images","tool","tools","free","online","fast","easy","secure",
  "next","back","previous","continue","cancel","ok","yes","no","submit","save","delete","edit","create",
  "login","logout","sign","signup","email","password","username","account","settings","help","support",
  "home","about","contact","privacy","terms","cookies","faq","blog","news",
  "loading","please","wait","error","success","warning","info","ready","done","finished","completed",
]);

const STOPWORDS_PT = new Set([
  "o","a","os","as","um","uma","uns","umas","de","da","do","das","dos","em","no","na","nos","nas",
  "para","pra","com","sem","por","ao","aos","à","às","pelo","pela","pelos","pelas",
  "é","são","foi","foram","ser","estar","está","estão","estava","tem","têm","tinha","tinham","há",
  "e","ou","mas","se","que","como","quando","onde","porque","então","também","já","ainda","só","apenas",
  "isso","isto","esse","esta","este","essa","aquilo","aquele","aquela","aqui","agora","depois","antes",
  "seu","sua","seus","suas","meu","minha","nosso","nossa","você","vocês","ele","ela","eles","elas","nós",
  "clique","clicar","enviar","baixar","arquivo","arquivos","imagem","imagens","ferramenta","ferramentas",
  "página","páginas","gratuito","grátis","rápido","fácil","seguro","online","próximo","voltar","anterior",
  "continuar","cancelar","sim","não","salvar","apagar","editar","criar","entrar","sair","cadastrar",
  "conta","ajuda","suporte","início","sobre","contato","privacidade","termos",
  "carregando","aguarde","erro","sucesso","aviso","pronto","concluído","finalizado",
  "comprimir","mesclar","dividir","rotacionar","desbloquear","proteger","numerar","converter","gerar",
  "comprima","converta","mescle","edite","reduza","gere","faça","deixe",
  "tamanho","qualidade","formato","entre","junte","corte","redimensione",
]);

const BRAND_TERMS = new Set([
  "pdf","jpg","jpeg","png","webp","heic","gif","bmp","svg","tiff",
  "json","xml","csv","html","css","js","ts","tsx","jsx",
  "excel","word","powerpoint","docx","xlsx","pptx",
  "qr","cnpj","cpf","cep","seo","url","api","cdn","cms","saas","ssl","tls","pwa",
  "mongodb","redis","minio","s3","aws","docker","node","react","next","tailwind","monkey",
]);
...
async function loadSeedUrls(): Promise<string[]> {
  ...
  try {
    ...
    const [tools, categories, pages] = await Promise.all([
      db.collection("tools").find({}, { projection: { link: 1, category: 1 } }).toArray(),
      db.collection("categories").find({}, { projection: { slug: 1, _id: 1 } }).toArray(),
      db.collection("pages").find({}, { projection: { slug: 1 } }).toArray(),
    ]);
...
function renderReport(reports: PageReport[]): string {
  ...
  if (withHits.length === 0) {
    ...
    lines.push("");
    lines.push(`> A heurística é estatística (stopwords EN vs PT). Frases curtas ou termos técnicos podem passar batido — confira manualmente os screenshots em \`.claude/i18n-screenshots/\`.`);
  } else {
  ...
  for (const r of reports) {
    ...
    lines.push(`- ${flag} [${r.status}] \`${r.url}\` — ${r.title || "(sem título)"} ${r.hits.length > 0 ? `— ${r.hits.length} bloco(s) suspeito(s)` : ""}`);
  }
  lines.push("");
  return lines.join("\n");
}
